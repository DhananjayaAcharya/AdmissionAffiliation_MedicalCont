...
{
    public class DentalRepositoryController : BaseController
    {
        private readonly ApplicationDbContext _context;


        public DentalRepositoryController(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
    ...
    string facultyCode)
        ...
        public async Task<IActionResult> TeachingFacultyDetails()
        {
            ...
            {
                return View(new List<DentalTeachingFacultyVm>());
            }
            ...
            // Build ViewModel
            // =========================
            var result = new List<DentalTeachingFacultyVm>();
            ...
            foreach (var dept in departments)
            {
                foreach (var desig in designations)
                {
                    ...
                    result.Add(new DentalTeachingFacultyVm
                    {
                        CollegeCode = collegeCode,
                        ...
                        // Required faculty from master
                        ExistingSeatIntake = existing?.RequiredIntake ?? requiredSeats.ToString(),

                        // Available faculty entered by user
                        PresentSeatIntake =
                            existing?.AvailableIntake ?? "0"
                    });
                }
            }
        ...
        public async Task<IActionResult> TeachingFacultyDetails( List<DentalTeachingFacultyVm> model)
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            ...
            if (model == null || !model.Any())
            {
                ...
                var formKeys = form != null
                    ? string.Join(", ", form.Keys)
                    : "No form keys";

                TempData["Error"] =
                    "No data received from form.";

                Console.WriteLine(
                    $"TeachingFacultyDetails POST failed. " +
                    $"College={collegeCode}, Keys={formKeys}");
            ...
            foreach (var item in model)
            {
                var existingRecord =
                    ...
                    .FirstOrDefaultAsync(x =>
                        ...
                        x.FacultyCode == item.FacultyCode &&
                ...
                if (existingRecord != null)
                {
                    ...
                    // Update Existing
                    // =========================
                    existingRecord.FacultyCode =
                        item.FacultyCode;

                    existingRecord.Designation =
                        item.DesignationName;

                    existingRecord.DesignationCode =
                        item.DesignationCode;

                    existingRecord.Department =
                        item.DepartmentName;

                    existingRecord.DepartmentCode =
                        item.DepartmentCode;

                    existingRecord.SeatSlabId =
                        item.SeatSlabId;

                    existingRecord.RequiredIntake =
                        item.ExistingSeatIntake ?? "0";

                    existingRecord.AvailableIntake =
                        item.PresentSeatIntake ?? "0";

                    _context.CollegeDesignationDetails
