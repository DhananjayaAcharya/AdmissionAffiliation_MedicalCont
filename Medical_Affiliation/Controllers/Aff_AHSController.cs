...
{
    public class Aff_AHSController : Controller
    {
        private readonly ApplicationDbContext _context;

        public Aff_AHSController(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        [HttpGet]
        public IActionResult AHS_BasicDetails()
        {
        ...
        public IActionResult AHS_BasicDetails(NursingInstituteDetailViewModel model)
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            var facultyCodeString = HttpContext.Session.GetString("FacultyCode");
            ...
            return RedirectToAction("AHS_UGPG");
        }



        [HttpGet]
        public IActionResult AHS_Institute_Report()
        {
        ...
        [HttpGet("AHS_UGPG")]
        public async Task<IActionResult> AHS_UGPG()
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode") ?? "";
            ...
            return View(new AHS_FacultyIntakeViewModel());
        }
        ...
        [HttpPost("AHS_UGPG")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> AHS_UGPG(AHS_FacultyIntakeViewModel model)
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            var facultyCode = HttpContext.Session.GetString("FacultyCode");
            ...
            return RedirectToAction(nameof(AHS_UGPG));
        }

        [HttpGet]
        public IActionResult AHS_ManageCourses()
        {
            ...
            var savedPG = (from u in _context.NursingUgpgdetails
                           ...
                           select new PGDetailsViewModel
                           {
                               ...
                               // No files loaded unless you need to display previews
                           }).ToList();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult AHS_ManageCourses(UGPGSSDetailsFormViewModel model)
        {
            string facultyCode = HttpContext.Session.GetString("FacultyCode");
            string collegeCode = HttpContext.Session.GetString("CollegeCode");
        ...
        [HttpGet]
        public IActionResult AHS_YearwiseMaterials()
        {
            try
            {
                var collegeCode = HttpContext.Session.GetString("CollegeCode");
            ...
            catch (Exception ex)
            {
                ...
                return View(new List<AHS_YearwiseMaterialsDataViewModel>());
            }
        }
        ...
        public IActionResult DeleteYearwiseMaterial(int parametersId)
        {
            ...
            return RedirectToAction("AHS_YearwiseMaterials");
        }

        [HttpPost]
        public IActionResult AHS_YearwiseMaterials(List<AHS_YearwiseMaterialsDataViewModel> model)
        {
            try
            {
                var collegeCode = HttpContext.Session.GetString("CollegeCode");
        ...
        //  [HttpGet]
        //  public IActionResult AHS_Affiliated_YearwiseMaterials()
        //  {
        //      try
        //      {
        //          var collegeCode = HttpContext.Session.GetString("CollegeCode");
        ...
        //          // Fetch all affiliated clinical parameters with existing yearwise data using LINQ
        //          var model = (from param in _context.MstNursingAffiliatedMaterialData
        //                       where param.FacultyCode == facultyCode
        //                       join yw in _context.AhsAffiliatedYearwiseMaterialsData
        ...
        //                       select new AHS_YearwiseMaterialsDataViewModel
        //                       {
        ...
        //          ViewBag.YearwiseData = _context.AhsAffiliatedYearwiseMaterialsData
        //.Where(y => y.CollegeCode == collegeCode && y.FacultyCode == facultyCode)
        //.Select(y => new
        //{
        ...
        //    // ✅ ADD THESE NEW LINES
        //    parentHospitalNABLDoc = y.ParentHospitalNabldoc,
        //    parentHospitalKSPCDoc = y.ParentHospitalKspcdoc,
        ...
        //          // Hardcoded dropdown list for hospital types
        ...
        public IActionResult DownloadDocument(int id, string type)
        {
            ...
            var record = _context.AhsAffiliatedYearwiseMaterialsData.FirstOrDefault(x => x.Id == id);
            ...
            switch (type)
            {
                case "MOU": fileBytes = record.ParentHospitalMoudoc; fileName = "MOU.pdf"; break;
                case "Owner": fileBytes = record.ParentHospitalOwnerNameDoc; fileName = "OwnerName.pdf"; break;
                case "KpmeBeds": fileBytes = record.ParentHospitalKpmebedsDoc; fileName = "KpmeBeds.pdf"; break;
                case "PostBasic": fileBytes = record.ParentHospitalPostBasicDoc; fileName = "PostBasic.pdf"; break;
                case "NABL": fileBytes = record.ParentHospitalNabldoc; fileName = "NABL.pdf"; break;
                case "KSPC": fileBytes = record.ParentHospitalKspcdoc; fileName = "KSPC.pdf"; break;
        ...
        public IActionResult DeleteYearwiseMaterial1(int parametersId)
        {
            ...
            return RedirectToAction("AHS_Affiliated_YearwiseMaterials");
        }
        [HttpGet]
        public IActionResult AHS_Affiliated_YearwiseMaterials()
        {
            try
            {
                ...
                // Left join base parameters with saved yearwise data
                var model = (from param in _context.MstNursingAffiliatedMaterialData
                             where param.FacultyCode == facultyCode
                             join yw in _context.AhsAffiliatedYearwiseMaterialsData
                                 .Where(y => y.CollegeCode == collegeCode && y.FacultyCode == facultyCode)
                             ...
                             select new AHS_YearwiseMaterialsDataViewModel
                             {
                                 ...
                                 CollegeCode = collegeCode,
                                 FacultyCode = facultyCode,
                                 Year1 = ywItem != null ? ywItem.Year1 : "0",
                                 Year2 = ywItem != null ? ywItem.Year2 : "0",
                                 Year3 = ywItem != null ? ywItem.Year3 : "0"
                             })
                             .ToList();

                // Saved rows for read-only table
                ViewBag.YearwiseData = _context.AhsAffiliatedYearwiseMaterialsData
                    .Where(y => y.CollegeCode == collegeCode && y.FacultyCode == facultyCode)
                    .Select(y => new
                    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult AHS_Affiliated_YearwiseMaterials(List<YearwiseMaterialViewModel> model)
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            var facultyCode = HttpContext.Session.GetString("FacultyCode");
            ...
            try
            {
                ...
                IFormFile GetFile(string name) => HttpContext.Request.Form.Files.GetFile(name);

                var kpmeFile = GetFile("ParentHospitalKpmebedsDocFile");
                var mouFile = GetFile("ParentHospitalMoudocFile");
                var ownerFile = GetFile("ParentHospitalOwnerNameDocFile");
                var postBasicFile = GetFile("ParentHospitalPostBasicDocFile");
                var nablFile = GetFile("ParentHospitalNABLDocFile");
                var kspcFile = GetFile("ParentHospitalKSPCDocFile");

                foreach (var item in model)
                {
                    // 1) Resolve parameters name from posted value, then try master tables
                    string parametersName = item?.ParametersName;
                    if (string.IsNullOrWhiteSpace(parametersName))
                    {
                        parametersName = _context.MstNursingAffiliatedMaterialData
                        ...
                        if (string.IsNullOrWhiteSpace(parametersName))
                        {
                            parametersName = _context.MstAffiliatedMaterialData
                                .Where(p => p.ParametersId == item.ParametersId)
                                .Select(p => p.ParametersName)
                                .FirstOrDefault();
                        }
                    ...
                    // 2) Defensive fallback (DB column is NOT NULL)
                    parametersName ??= $"Param-{item?.ParametersId ?? 0}";

                    // 3) Find existing record
                    var existing = _context.AhsAffiliatedYearwiseMaterialsData
                        .FirstOrDefault(y => y.CollegeCode == collegeCode
                    ...
                    if (existing != null)
                    {
                        ...
                        if (kpmeFile?.Length > 0) existing.ParentHospitalKpmebedsDoc = ConvertFileToBytes(kpmeFile);
                        if (mouFile?.Length > 0) existing.ParentHospitalMoudoc = ConvertFileToBytes(mouFile);
                        if (ownerFile?.Length > 0) existing.ParentHospitalOwnerNameDoc = ConvertFileToBytes(ownerFile);
                        if (postBasicFile?.Length > 0) existing.ParentHospitalPostBasicDoc = ConvertFileToBytes(postBasicFile);
                        if (nablFile?.Length > 0) existing.ParentHospitalNabldoc = ConvertFileToBytes(nablFile);
                        if (kspcFile?.Length > 0) existing.ParentHospitalKspcdoc = ConvertFileToBytes(kspcFile);

                        _context.AhsAffiliatedYearwiseMaterialsData.Update(existing);
                    }
                    else
                    {
                        var newRecord = new AhsAffiliatedYearwiseMaterialsDatum
                        {
                            CollegeCode = collegeCode,
                            FacultyCode = facultyCode,
                            ParametersId = item.ParametersId,
                            ...
                            ParentHospitalKpmebedsDoc = kpmeFile?.Length > 0 ? ConvertFileToBytes(kpmeFile) : null,
                            ParentHospitalMoudoc = mouFile?.Length > 0 ? ConvertFileToBytes(mouFile) : null,
                            ParentHospitalOwnerNameDoc = ownerFile?.Length > 0 ? ConvertFileToBytes(ownerFile) : null,
                            ParentHospitalPostBasicDoc = postBasicFile?.Length > 0 ? ConvertFileToBytes(postBasicFile) : null,
                            ParentHospitalNabldoc = nablFile?.Length > 0 ? ConvertFileToBytes(nablFile) : null,
                            ParentHospitalKspcdoc = kspcFile?.Length > 0 ? ConvertFileToBytes(kspcFile) : null
                        };

                        _context.AhsAffiliatedYearwiseMaterialsData.Add(newRecord);
                    }
                }
            ...
            {
                tx.Rollback();
                // surface inner exception for diagnosis
                var inner = dbEx.InnerException?.Message ?? dbEx.Message;
                Console.WriteLine($"DB error saving AHS_Affiliated_YearwiseMaterials: {inner}");
                TempData["Error"] = "Database error saving data: " + (dbEx.InnerException?.Message ?? dbEx.Message);
            }
            catch (Exception ex)
            {
                tx.Rollback();
                Console.WriteLine($"Error saving AHS_Affiliated_YearwiseMaterials: {ex}");
        ...
        [HttpGet]
        public IActionResult AHS_FacultyDetails()
        {
            string collegeCode = HttpContext.Session.GetString("CollegeCode");
            string facultyCode = HttpContext.Session.GetString("FacultyCode");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult AHS_FacultyDetails(IList<FacultyDetailsViewModel> model)
        {
            string collegeCode = HttpContext.Session.GetString("CollegeCode");
            string facultyCode = HttpContext.Session.GetString("FacultyCode");
                ...
                model = new List<FacultyDetailsViewModel>
     {
          new FacultyDetailsViewModel
          {
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    return RedirectToAction("AHS_ExamResults");
                }
                catch (Exception ex)
                {
                    transaction.Rollback();
        ...
        public async Task<IActionResult> AHS_TeachingFacultyDetails()
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            var facultyCode = HttpContext.Session.GetString("FacultyCode");
        ...
        //[ValidateAntiForgeryToken]
        //[Authorize(AuthenticationSchemes = "CollegeAuth")]
        public async Task<IActionResult> AHS_TeachingFacultyDetails(List<TeachingFacultyViewModel> model)
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            ...
            return RedirectToAction("AHS_FacultyDetails"); // reload page
        }
        ...
        [HttpGet]
        public IActionResult AHS_ManageDesignationIntake()
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            var facultyCode = HttpContext.Session.GetString("FacultyCode");
        ...
        [HttpPost]
        public IActionResult AHS_ManageDesignationIntake(List<CollegeDesignationDetailsViewModel> model)
        {
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            var facultyCode = HttpContext.Session.GetString("FacultyCode");
            ...
            foreach (var item in model)
            {
                ...
                string requiredIntakeValue = item.PresentIntake.ToString() ?? "0"; // Set to default if null

                if (existing != null)
                {
                    existing.AvailableIntake = item.PresentIntake.ToString();
                    existing.RequiredIntake = requiredIntakeValue; // <--- Add this line!
                ...
                else
                {
                    _context.CollegeDesignationDetails.Add(new CollegeDesignationDetail
                    {
                        ...
                        RequiredIntake = requiredIntakeValue, // <--- Add this line!
            ...
            _context.SaveChanges();
            return RedirectToAction("AHS_FacultyDetails");
        }

        //------------------------------------ 6th page -----------------------------------------

        [HttpGet]
        public async Task<IActionResult> AHS_ExamResults()
        {
            var facultyCodeStr = HttpContext.Session.GetString("FacultyCode");
            var collegeCode = HttpContext.Session.GetString("CollegeCode");
            ...
            var model = new List<AHSExamResultViewModel>
    {
        new() { Course = "B.Sc", Year = "1st Year" },
        new() { Course = "B.Sc", Year = "2nd Year" },
        new() { Course = "B.Sc", Year = "3rd Year" },

        new() { Course = "Bachelors", Year = "1st Year" },
        new() { Course = "Bachelors", Year = "2nd Year" },
        new() { Course = "Bachelors", Year = "3rd Year" },

        new() { Course = "Masters", Year = "1st Year" },
        new() { Course = "Masters", Year = "2nd Year" }
    };
        ...
        [HttpPost]
        public async Task<IActionResult> AHS_ExamResults(List<AHSExamResultViewModel> model)
        {
            try
            {
                var facultyCodeStr = HttpContext.Session.GetString("FacultyCode");
                ...
                if (model == null || !model.Any())
                {
                    ...
                    return View(model ?? new List<AHSExamResultViewModel>());
                }
    ...
    .ToList();
                ...
                return RedirectToAction(nameof(AHS_ExamResults));
            }
            catch (Exception ex)
            {
